...
# $1 = folder path that you will feed into tar, $2 = webhook url (optional
...
function upload_subprocess(){ # \$1 = part no but started at 0, \$2 = offset
    ...
    do
        link=\`tail -c +\$offset /tmp/barbruh114514 | head -c \$sizeperpart | formdata_pipeline "\$checksum.txt" | upload \$partno\`
    done
    echo "\$link|\$checksum"
}
...
function dir_upload() { # $1 = dirname
    tar -cv "$1" | split -b $((filesize_per_process*processes)) --filter "cat | bash /tmp/tempbitte114514.sh >> /tmp/wiebitte114514.txt; sleep 1"
}

cat /dev/null > /tmp/wiebitte114514.txt
dir_upload "$1"
finallink=`cat /tmp/wiebitte114514.txt | bash /tmp/tempbitte114514.sh`
finallink2=`echo "$finallink" | cut -d\| -f1`
size=`du -b --max-depth=0 "$1" | awk '{ print $1 }'`
[ "$2" ] && webhookurl="$2"
curl -k --connect-timeout 20 --retry 20 --retry-delay 0 --retry-max-time 40 -F "payload_json={\"content\":\"link of **$1** ($size bytes): $finallink2\",\"username\":\"EulaAAAAAAAA\",\"avatar_url\":\"https://cdn.discordapp.com/attachments/524633631012945922/937727442171342868/eulaAAAAAAAA.png\"}" "$webhookurl" # || 
echo "link of $1 ($size bytes): $finallink2" >> ~/localdb.txt
